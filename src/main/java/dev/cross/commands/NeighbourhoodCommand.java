...
@Extension
public class NeighbourhoodCommand implements CommandHandler {
    @Override
    public Command command() {
        return new Command("neighbourhood") {
            @Override
            public boolean execute(net.minecraft.src.command.CommandHandler commandHandler, CommandSender sender, String[] args) {
                if (!Config.get().isGlobalHomes()) {
                    sender.sendMessage(ChatColor.red + "Neighbourhood is not allowed on this server.");
                    return true;
                }
                if (args.length == 0) {
                    sender.sendMessage(ChatColor.orange + "Neighbours:");
                ...
                if (args.length == 1) {
                    ...
                    if (tp == null) { sender.sendMessage(ChatColor.red + "Neighbour not found.");return true; }
            ...
            public void sendCommandSyntax(net.minecraft.src.command.CommandHandler commandHandler, CommandSender commandSender) {
                commandSender.sendMessage("/neighbourhood <name>");
            }
        };
    }
}
